...
export function useVocabularyProcessor() {
  ...
  useEffect(() => {
    ...
    // Only process USER messages that are DONE (complete) and have text content
    if (
      ...
      // Only add user messages to batch buffer when they're complete
      addMessageToBuffer(latestItem.itemId, latestItem.title, 'user');

      // Log that message was added to buffer (hidden from UI)
      console.log(`Auto-added complete user message to batch buffer: ${latestItem.itemId.slice(0, 8)}...`);
    }

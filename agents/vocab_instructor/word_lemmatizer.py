...
class WordLemmatizer:
    ...
    def _create_fallback_rules(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            'ting': ['t'],   # sitting -> sit, getting -> get
            'ning': ['n'],   # running -> run, beginning -> begin
            'ming': ['m'],   # swimming -> swim, humming -> hum
            'pping': ['p'],  # stopping -> stop, shopping -> shop
            'gging': ['g'],  # jogging -> jog, hugging -> hug
            'lling': ['l'],  # calling -> call, falling -> fall
            'ssing': ['ss'], # passing -> pass, missing -> miss

            # Past tense forms
            'ied': ['y'],    # carried -> carry, studied -> study
            'pped': ['p'],   # stopped -> stop, dropped -> drop
            'gged': ['g'],   # jogged -> jog, hugged -> hug
            'lled': ['l'],   # called -> call, filled -> fill
            'ssed': ['ss'],  # passed -> pass, missed -> miss
            'tted': ['t'],   # chatted -> chat, fitted -> fit

            # Regular verb forms
            'ing': ['e', ''],  # making -> make, running -> run
            'ed': ['e', ''],   # baked -> bake, walked -> walk
            ...
            'ches': ['ch'],    # watches -> watch, beaches -> beach
            'shes': ['sh'],    # dishes -> dish, brushes -> brush
            'xes': ['x'],      # boxes -> box, fixes -> fix
            'oes': ['o'],      # tomatoes -> tomato, heroes -> hero

            # Past participle forms
            'en': [''],        # taken -> take, broken -> break
            'n': [''],         # seen -> see, been -> be

            # Adjective/adverb forms
            'ier': ['y'],      # happier -> happy, easier -> easy
            'iest': ['y'],     # happiest -> happy, easiest -> easy
            'er': [''],        # bigger -> big, faster -> fast
            'est': [''],       # biggest -> big, fastest -> fast
            'ly': [''],        # quickly -> quick, slowly -> slow

            # Additional verb forms
            'ize': [''],       # realize -> real, organize -> organ
            'ise': [''],       # realise -> real, organise -> organ
            'fy': [''],        # simplify -> simpl, beautify -> beauti

            # Gerund forms with 'e' dropping
            'ving': ['ve'],    # having -> have, giving -> give
            'cing': ['ce'],    # dancing -> dance, placing -> place
            'ging': ['ge'],    # changing -> change, judging -> judge
            'zing': ['ze'],    # amazing -> amaze, freezing -> freeze
            'sing': ['se'],    # using -> use, choosing -> choose
        }
